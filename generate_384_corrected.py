#!/usr/bin/env python3
"""
Corrected 384-Dimension Embedding Generation

This script generates embeddings using all-MiniLM-L6-v2 model (384 dimensions)
with proper vector storage format.
"""
...
class Corrected384EmbeddingGenerator:
    """Generate 384-dimension embeddings with proper vector storage"""
    ...
    def connect_to_supabase(self) -> bool:
        ...
        try:
            ...
            # Test embedding dimensions with proper vector format
            test_embedding = [float(x) for x in np.random.rand(384)]  # Ensure float type
            ...
            if result.data:
                ...
                try:
                    ...
                    verify_result = self.supabase.table('nelson_textbook_chunks')\
                        .select('embedding')\
                        .eq('id', test_id)\
                        .execute()
                    
                    if verify_result.data and verify_result.data[0]['embedding']:
                        stored_embedding = verify_result.data[0]['embedding']
                        if isinstance(stored_embedding, list) and len(stored_embedding) == 384:
                            logger.info("✅ Table accepts 384-dimension vector embeddings correctly")
                        else:
                            logger.error(f"❌ Stored embedding format issue: {type(stored_embedding)}, length: {len(stored_embedding) if hasattr(stored_embedding, '__len__') else 'N/A'}")
                            return False
    ...
    def get_records_batch(self, limit: int = 25, offset: int = 0) -> List[Dict[str, Any]]:
    ...
    def update_embeddings_batch(self, records: List[Dict[str, Any]], embeddings: np.ndarray) -> bool:
        """Update records with their embeddings using proper vector format"""
        try:
            success_count = 0
            ...
            for i, record in enumerate(records):
                ...
                try:
                    # Convert to proper float list format
                    embedding_vector = [float(x) for x in embeddings[i]]
                    
                    # Verify dimensions
                    if len(embedding_vector) != 384:
                        logger.error(f"❌ Wrong embedding dimension: {len(embedding_vector)}, expected 384")
                        continue
    ...
    def process_all_embeddings(self) -> bool:
        ...
        logger.info("🚀 Starting corrected 384D embedding generation")
        
        self.stats['start_time'] = datetime.now()
        
        try:
            offset = 0
            batch_limit = 25  # Smaller batches for reliability
            consecutive_empty_batches = 0
            
            while consecutive_empty_batches < 5:  # Stop after 5 consecutive empty batches
    ...
    def print_summary(self):
        ...
        print("\n📊 CORRECTED 384D EMBEDDING GENERATION SUMMARY")
        print("=" * 60)
        print(f"🤖 Model: {self.model_name}")
        print(f"📐 Dimensions: 384 (native, corrected format)")
        ...
        if verification:
            ...
            if verification['completion_rate'] >= 95:
                print(f"\n🎉 CORRECTED 384D EMBEDDING GENERATION SUCCESSFUL!")
...
def main():
    ...
    print("🔧 NELSON PEDIATRICS CORRECTED 384D EMBEDDING GENERATION")
    print("=" * 70)
    print("🤖 Using all-MiniLM-L6-v2 model (native 384 dimensions)")
    print("✅ Corrected vector storage format")
    print("🛡️ Robust error handling and data validation")
    print("=" * 70)
    
    # Initialize generator
    generator = Corrected384EmbeddingGenerator(batch_size=25)
    
    print(f"\n🤖 CONFIGURATION:")
    print(f"  📋 Model: {generator.model_name}")
    print(f"  📐 Dimensions: 384 (native)")
    print(f"  📦 Batch size: {generator.batch_size}")
    print(f"  ✅ Features: Corrected vector format, data validation, individual updates")
    ...
    if success:
        print("\n" + "=" * 70)
        print("🎉 CORRECTED 384D EMBEDDING GENERATION COMPLETE!")
        print("=" * 70)
        print("🚀 Your Nelson Pediatrics database supports semantic search")
        print("✅ Proper vector format ensures compatibility")
    else:
        print("\n" + "=" * 70)
