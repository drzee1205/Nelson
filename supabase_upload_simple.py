#!/usr/bin/env python3
"""
Nelson Pediatrics - Simple Supabase Upload

This script uploads your Nelson Pediatrics data to Supabase,
handling the embedding dimension issue automatically.
"""
...
def transform_data_for_supabase(data: List[Dict]) -> List[Dict]:
    ...
    logger.info(f"✅ Transformed {len(transformed_data)} records for Supabase (without embeddings)")
...
def main():
    ...
    print("🏥 Nelson Pediatrics - Simple Supabase Upload")
    print("=" * 60)
    print("📊 Uploading to table: nelson_textbook_chunks")
    print("🌐 Supabase URL: https://nrtaztkewvbtzhbtkffc.supabase.co")
    print("⚠️ Note: Uploading without embeddings due to dimension mismatch")
    print("=" * 60)
    ...
    try:
        ...
        print(f"⚠️ Vector search: Disabled (embeddings not uploaded)")
        print(f"💾 Storage: Cloud (persistent)")
        
        print("\n🚀 Next Steps:")
        print("1. Fix the embedding dimension issue in your Supabase table")
        print("2. Re-upload with embeddings for vector similarity search")
        print("3. Test the API endpoints with your NelsonGPT")
        
        print("\n💡 To enable vector search:")
        print("1. Update your table schema to use vector(384) instead of vector(1536)")
        print("2. Run the upload script again to include embeddings")
        
        print("\n🏥 Your Nelson Pediatrics text data is now in Supabase! ⚡")
